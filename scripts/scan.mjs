...
if (args.includes('--only') && !only) { console.error('--only needs a source id'); process.exit(2) }
if (args.includes('--now') && Number.isNaN(Date.parse(fixedNow ?? ''))) { console.error('--now needs an ISO time'); process.exit(2) }
if (only && !USED.some(e => e.id === only)) { console.error(`--only ${only}: not a used source (${USED.map(e => e.id).join(', ')})`); process.exit(2) }
...
function readPrevious () {
  try { return JSON.parse(readFileSync(SOURCES_JSON, 'utf8')) } catch { return { sources: [] } }
}
...
async function once ({ forceNow }) {
  let health = {}
  if (!dry) {
    try { health = await workerHealth() } catch (err) {
  ...
  log(`[scan ${stamp()}] ${dry ? 'dry run (no POST)' : `→ ${WORKER}`}${only ? ` only ${only}` : ''}${forceNow ? ' forced' : ''}${mapped ? ' [origins mapped: sample]' : ''}`)
  ...
  for (const s of scan.sources) {
    if (s.result === 'skipped') { log(`  ${s.source_id.padEnd(18)} skipped (${s.skipped_reason})`); continue }
    const bytes = s.requests.map(r => r.bytes).join('+')
    log(`  ${s.source_id.padEnd(18)} ${s.result.padEnd(14)} list ${String(s.list_date ?? '-').padEnd(10)} notices ${String(s.notices.length).padStart(2)}  dropped ${s.quotes_dropped}  bytes ${bytes || 0}${s.error ? `  (${s.error})` : ''}`)
    if (!dry) {
      try {
        const r = await ingest(s)
        log(`  ${''.padEnd(18)} ingested: accepted ${r.accepted}, dropped ${r.quotes_dropped}, removed ${r.removed}, current ${r.notices_current}`)
      } catch (err) {
  ...
  if (dry) {
    const lite = { ...scan, sources: scan.sources.map(s => (s.raws ? { ...s, raws: s.raws.map(({ body, ...r }) => ({ ...r, bytes: Buffer.byteLength(body) })) } : s)) }
...
if (!watch) {
  ...
  const tick = async () => {
    if (busy) return
    busy = true
    try { await once({ forceNow: first && force }) } catch (err) { console.error(`[scan] ${err.message}`) }
    first = false
    busy = false
  }
