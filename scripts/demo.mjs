...
function start(name, cmd, args, opts = {}) {
  ...
  child.on('exit', (code) => {
    ...
    if (!stopping && longLived) {
      log(`${name} exited (${code}); stopping`)
      stop(1)
    }
  })
  return child
...
const runOnce = (name, cmd, args, opts = {}) =>
  new Promise((resolve) => {
...
process.on('SIGINT', () => {
  log('Ctrl-C: stopping the Worker, the app and the scanner')
  stop(0)
})
...
for (const [what, port] of [
  ...
  if (await portInUse(port)) {
    log(`port ${port} (${what}) is already in use; stop that first`)
    process.exit(1)
  }
}
...
const mig = await runOnce('migrations', 'wrangler', ['d1', 'migrations', 'apply', 'school-closures-watch', '--local'], {
  cwd: ROOT + 'worker',
  env: wEnv,
})
if (mig !== 0) {
  log(`migrations failed (${mig})`)
  stop(1)
}

log(`starting the Worker on ${WORKER_PORT}`)
start('worker', 'wrangler', ['dev', '--local', '--ip', '127.0.0.1', '--port', String(WORKER_PORT)], {
  cwd: ROOT + 'worker',
  env: wEnv,
  longLived: true,
})
...
if (!(await waitFor(`${WORKER_URL}/api/health`, 90000))) {
  log('the Worker did not start')
  stop(1)
}
