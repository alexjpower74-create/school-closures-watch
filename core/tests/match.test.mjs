...
test('the six real row schools match exactly on name + community', () => {
  const rows = [
    ['Glovertown Academy', 'Glovertown, NL'], ['Eastside Elementary', 'Corner Brook, NL'],
    ["Bay d'Espoir Academy", "St. Alban's, NL"], ['Bishop White School', 'Port Rexton, NL'],
    ["St. Mark's School", "King's Cove, NL"], ['Upper Gullies Elementary', 'Conception Bay South, NL']
  ]
  for (const [school_text, community_text] of rows) {
    assert.deepEqual(matchRow({ school_text, community_text }, NLS),
      { matches: [{ school_id: idOf(school_text), how: 'exact', reason: null }], unmatched_reason: null }, school_text)
  }
})
...
test('may_apply: "SAMPLE Glovertown" / "Gander, NL" → Glovertown Academy, name_similar (never exact)', () => {
  assert.deepEqual(matchRow({ school_text: 'SAMPLE Glovertown', community_text: 'Gander, NL' }, NLS),
    { matches: [{ school_id: GLOVERTOWN, how: 'may_apply', reason: 'name_similar' }], unmatched_reason: null })
})
...
test('may_apply: a name shared by more than one school → name_shared', () => {
  ...
  assert.deepEqual(matchRow({ school_text: 'SAMPLE Twin School', community_text: 'Gamma, NL' }, schools).matches.map(m => [m.school_id, m.how, m.reason]),
    [['a', 'may_apply', 'name_shared'], ['b', 'may_apply', 'name_shared']])
...
test('similar names: 1–5 candidates may_apply, more than 5 → too_many, shared token + same community', () => {
  const many = n => Array.from({ length: n }, (_, i) => ({ id: `h${i}`, name: `SAMPLE Harbour Elementary ${i + 1}`, community: `Place ${i}`, coverage: 'nlschools' }))
  assert.equal(matchRow({ school_text: 'SAMPLE Harbour', community_text: 'Elsewhere, NL' }, many(5)).matches.length, 5)
  assert.deepEqual(matchRow({ school_text: 'SAMPLE Harbour', community_text: 'Elsewhere, NL' }, many(6)), { matches: [], unmatched_reason: 'too_many' })
...
test('unmatched row → no_school; a row naming the whole Central region → scope region', () => {
  assert.deepEqual(matchRow({ school_text: 'SAMPLE Nowhere Harbour School', community_text: 'Nowhere, NL' }, NLS), { matches: [], unmatched_reason: 'no_school' })
  const base = { source_id: 'nlschools-status', kind: 'school_row', row_id: '1', list_date: '2026-09-14', status_class: 'closedAllDay', status_text: 'CLOSED ALL DAY', quote: 'SAMPLE Closed due to storm' }
  ...
  const ex = classifyNotice({ ...base, school_text: 'Glovertown Academy', community_text: 'Glovertown, NL', quote: 'SAMPLE like all schools in the Central region' }, SCHOOLS).notice
  assert.equal(ex.scope, 'school')
})

test('classifyNotice: text notices and feed posts', () => {
  const t = classifyNotice({ source_id: 'nlschools-notices', kind: 'text_notice', row_id: 'x', quote: 'SAMPLE All schools in the Central region are closed for the day' }, SCHOOLS).notice
  assert.deepEqual([t.status, t.status_basis, t.status_evidence, t.scope, t.scope_region], ['closed', 'phrase', 'closed for the day', 'region', 'central'])
  const d = classifyNotice({ source_id: 'nlschools-notices', kind: 'text_notice', row_id: 'y', quote: 'SAMPLE Parent-teacher night moved' }, SCHOOLS).notice
  assert.deepEqual([d.status, d.scope], ['other', 'district'])
  const named = classifyNotice({ source_id: 'csfp-news', kind: 'feed_post', row_id: 'z', source_text: "SAMPLE École Boréale fermée aujourd'hui en raison de la tempête" }, SCHOOLS).notice
  ...
  const none = classifyNotice({ source_id: 'csfp-news', kind: 'feed_post', row_id: 'z', source_text: 'SAMPLE Écoles fermées' }, SCHOOLS).notice
