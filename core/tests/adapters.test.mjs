...
const rawsFor = (adapter, bodies) => adapter.fetchPlan(NOW).map((p, i) => ({
  ...p, raw_ref: `${adapter.id}/2026-09-14T16-40-05-000Z-${p.name}.${p.format}`, fetched_at: NOW, body: bodies[i]
...
test('registry: §2 ids, kinds and fetch order; link-only and not-used have nothing to fetch', () => {
  assert.deepEqual(REGISTRY.map(e => [e.id, e.kind]), [
    ['nlschools-status', 'used'], ['nlschools-notices', 'used'], ['csfp-news', 'used'],
    ['nlschools-busplanner', 'link_only'], ['nlschools-social', 'link_only'], ['nlschools-weather-protocol', 'link_only'],
    ['csfp-transport', 'link_only'], ['radio-aggregators', 'not_used']
  ])
  assert.deepEqual(planFor(REGISTRY_BY_ID['nlschools-status']).map(p => p.url), [
...
test('real 2026-09-14: two rows, list date, open rule, Glovertown closed exact, Eastside other exact', () => {
  ...
  assert.deepEqual(
    ...
    ['468', 'Glovertown, NL', 'closedAllDay', 'CLOSED ALL DAY', 'closed', 'School closed all day NOTE: Water Shut Off', 'FOS 05', 'CENTRAL', 'school', 'school_row', 'https://www.nlschools.ca/schools/statusreport.jsp'])
  ...
  assert.deepEqual(g.raw_refs, raws.map(r => r.raw_ref))
  ...
  assert.deepEqual([e.community_text, e.status_class, e.status_text, e.status, e.status_basis], ['Corner Brook, NL', 'otherStatus', 'OTHER STATUS', 'other', 'class'])
...
test('wayback 2024-01-10: Bay d\'Espoir closed_part; Bishop White and St. Mark\'s delayed; all exact', () => {
  ...
  assert.equal(res.result, 'ok')
  assert.deepEqual(res.notices.map(n => [n.school_text, n.community_text, n.status_class, n.status, n.matches[0].school_id, n.matches[0].how]), [
...
test('wayback 2024-01-17: Eastside Elementary closedForPD → closed; 2025-07-31: Upper Gullies closingEarly → early_dismissal', () => {
  const pd = nlsStatus.parse(rawsFor(nlsStatus, [undefined, sample('wayback/schoolstatus-20240117055404.html')]), ctx({ list_date: '2024-01-17' }))
  assert.deepEqual(pd.notices.map(n => [n.school_text, n.status_class, n.status, n.matches[0].how]), [['Eastside Elementary', 'closedForPD', 'closed', 'exact']])
  assert.equal(pd.notices[0].quote, 'School closed all day for teacher PD')
  const early = nlsStatus.parse(rawsFor(nlsStatus, [undefined, sample('wayback/schoolstatus-20250731230821.html')]), ctx({ list_date: '2025-07-31' }))
  assert.deepEqual(early.notices.map(n => [n.school_text, n.community_text, n.status, n.matches[0].school_id, n.matches[0].how]),
...
test('SAMPLE storm fragment: region row, ambiguous, same name other community, unmatched, unmapped class, skipped row', () => {
  ...
  assert.deepEqual([region.status, region.scope, region.scope_region, region.scope_evidence, region.community_text, region.matches],
...
test('important notices: SAMPLE text notice naming the whole Central region', () => {
  ...
  assert.deepEqual([n.kind, n.status, n.status_basis, n.status_evidence, n.scope, n.scope_region, n.scope_evidence, n.title],
...
test('important notices: SAMPLE links → one notice per link ≥ 3 chars, absolute hrefs, district when no region', () => {
  ...
  const res = nlsNotices.parse(raws, ctx({ sample: true }))
  assert.deepEqual(res.notices.map(n => [n.title, n.link, n.status, n.scope, n.scope_region]), [
    ['SAMPLE Delayed opening for all schools in the Avalon region', 'https://www.nlschools.ca/about/news/sample-1.jsp', 'delayed', 'region', 'avalon'],
...
test('CSFP SAMPLE post naming École Boréale 2 h before now → 1 notice, may_apply to École Boréale only', () => {
  ...
  const n = res.notices[0]
  assert.deepEqual([n.kind, n.status, n.status_basis, n.scope, n.quote, n.posted_text, n.posted_at, n.link, n.list_date],
    ['feed_post', 'may_apply', 'none', 'board', "SAMPLE École Boréale fermée aujourd’hui en raison de la tempête",
      'Mon, 14 Sep 2026 16:00:00 +0000', '2026-09-14T16:00:00.000Z', 'https://csfp.nl.ca/blog/2026/09/14/sample-boreale-fermee/', null])
