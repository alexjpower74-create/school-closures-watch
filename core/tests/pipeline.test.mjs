...
before(async () => {
  fx = await startFixtureServer({ port: PORT })
})
after(async () => {
  await fx?.close()
})
...
test('forced scan of the real 2026-09-14 files: three used sources, ingest payloads, raw copies', async () => {
  ...
  assert.deepEqual(
    res.sources.map((s) => [s.source_id, s.result]),
    [
      ['nlschools-status', 'ok'],
      ['nlschools-notices', 'ok'],
      ['csfp-news', 'ok'],
    ],
  )
  ...
  assert.deepEqual(
    st.notices[0].raw_refs,
    st.raws.map((r) => r.raw_ref),
  )
...
test('every request carries the research User-Agent; order per host is page, fragment, notices', async () => {
  ...
  assert.ok(
    fx.hits.every((h) => h.ua === USER_AGENT),
    JSON.stringify(fx.hits.map((h) => h.ua)),
  )
...
test('due sources only unless force; --only scans one source', async () => {
  ...
  assert.deepEqual(
    later.sources.map((s) => s.result),
    ['ok', 'ok', 'ok'],
  )
  fx.reset()
  const one = await scan({ only: 'csfp-news' })
  assert.deepEqual(
    one.sources.map((s) => s.source_id),
    ['csfp-news'],
  )
  assert.deepEqual(
    fx.hits.map((h) => h.path),
    ['/feed/'],
  )
})
...
test('verification runs in runScan: a tampered quote is dropped and counted', async () => {
  ...
  const tampering = {
    ...orig,
    parse: (raws, ctx) => {
      const r = orig.parse(raws, ctx)
      r.notices[0] = { ...r.notices[0], quote: 'SAMPLE School open as usual' }
      return r
    },
  }
...
test('scan.mjs --dry --force: data/sources.json (registry + last results), scan-latest.json without bodies, raw cache', async () => {
  ...
  const dir = tmpData()
  try {
    const r = await runScript(['--dry', '--force', '--now', NOW], {
      SOURCE_ORIGIN_MAP: JSON.stringify(fx.originMap),
      SC_DATA_DIR: dir,
      SC_WORKER_URL: 'http://127.0.0.1:9',
    })
  ...
  const server = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      if (req.url === '/api/sources')
        return res.end(JSON.stringify({ sources: REGISTRY.map((e) => ({ id: e.id, kind: e.kind, last_attempt_at: lastAttempt })) }))
      if (req.url === '/api/admin/ingest') {
        ...
        return res.end(
          JSON.stringify({
            ...
            accepted: p.notices.length,
            quotes_dropped: 0,
            removed: 0,
            notices_current: p.notices.length,
          }),
        )
      }
      res.end('{}')
...
test('scan.mjs POSTs one ingest per scanned source with the admin token', async () => {
  ...
  const dir = tmpData()
  try {
    const r = await runScript(['--now', NOW], {
      SOURCE_ORIGIN_MAP: JSON.stringify(fx.originMap),
      SC_DATA_DIR: dir,
      SC_WORKER_URL: w.url,
      SC_ADMIN_TOKEN: 'sample-token',
    })
    assert.equal(r.code, 0, r.stderr)
    assert.deepEqual(
      w.ingests.map((i) => i.payload.source_id),
      ['nlschools-status', 'nlschools-notices', 'csfp-news'],
    )
    assert.ok(w.ingests.every((i) => i.auth === 'Bearer sample-token'))
    const st = w.ingests[0].payload
    assert.deepEqual(
      [st.trigger, st.result, st.sample, st.notices.length, st.raws.length, 'requests' in st],
      ['npm-scan', 'ok', true, 2, 2, false],
    )
...
test('storm scenario: real 2024-01-10 rows plus SAMPLE rows and the SAMPLE region-wide text notice', async () => {
  ...
  const st = byId(res, 'nlschools-status')
  assert.deepEqual(
    st.notices.map((n) => [n.school_text, n.status, n.scope]),
    [
      ["Bay d'Espoir Academy", 'closed_part', 'school'],
      ['Bishop White School', 'delayed', 'school'],
      ["St. Mark's School", 'delayed', 'school'],
      ['SAMPLE Glovertown', 'closed', 'school'],
      ['SAMPLE Nowhere Harbour School', 'closed', 'unmatched'],
    ],
  )
  const tn = byId(res, 'nlschools-notices').notices
  assert.deepEqual(
    tn.map((n) => [n.status, n.scope, n.scope_region]),
    [['closed', 'region', 'central']],
  )
})
