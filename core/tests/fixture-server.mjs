...
export function startFixtureServer ({ port }) {
  ...
  const server = createServer((req, res) => {
    ...
    const ctl = (code, obj) => { res.writeHead(code, { 'content-type': 'application/json' }); res.end(obj ? JSON.stringify(obj) : '') }
    if (u.pathname === '/__log') return ctl(200, { scenario, hits })
    if (u.pathname === '/__reset') { hits.length = 0; failing.clear(); hanging.clear(); scenario = 'today'; return ctl(200, { ok: true }) }
    ...
    if (u.pathname === '/__fail') { failing.add(u.searchParams.get('path')); return ctl(200, { ok: true }) }
    if (u.pathname === '/__hang') { hanging.add(u.searchParams.get('path')); return ctl(200, { ok: true }) }
    ...
    if (failing.has(u.pathname)) { res.writeHead(503, { 'content-type': 'text/plain' }); return res.end('fixture failure') }
    const out = scenarioBody(scenario, u.pathname)
    if (!out) { res.writeHead(404, { 'content-type': 'text/plain' }); return res.end('not in fixtures') }
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, '127.0.0.1', () => resolve({
      ...
      setScenario (name) { if (!SCENARIOS[name]) throw new Error(`unknown scenario ${name}`); scenario = name },
      fail (path) { failing.add(path) },
      hang (path) { hanging.add(path) },
      reset () { hits.length = 0; failing.clear(); hanging.clear(); scenario = 'today' },
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  startFixtureServer({ port }).then(f => console.log(`fixture server on ${f.origin}`), err => { console.error(err.message); process.exit(1) })
}
