...
test('§4.1 table: codes, labels and ranks', () => {
  assert.deepEqual(STATUSES.map(s => [s.code, s.label, s.rank]), [
    ['closed', 'Closed', 1], ['closed_part', 'Closed part of the day', 2], ['buses_cancelled', 'Buses cancelled', 3],
    ['early_dismissal', 'Closing early', 4], ['delayed', 'Delayed opening', 5], ['buses_delayed', 'Buses delayed', 6],
    ['other', 'Other notice, read it', 7], ['may_apply', 'A notice may apply', 8], ['unknown', 'Unknown', 9],
    ['open', 'Open, no notice', 10]
  ])
})

test('§4.1 NLSchools classes decide the status', () => {
  const want = { closedAllDay: 'closed', closedForPD: 'closed', closedForHoliday: 'closed', closedForMorning: 'closed_part',
    closedForAfternoon: 'closed_part', closingEarly: 'early_dismissal', delayedOpening: 'delayed', busDelayed: 'buses_delayed',
    otherStatus: 'other' }
...
test('§4.1 STATUS text only when the class is not in the table; unmapped classes reported', () => {
  ...
  assert.deepEqual(t('CLOSED ALL DAY'), { status: 'closed', status_basis: 'status_text', status_evidence: 'CLOSED ALL DAY', unmapped_class: 'snowDay' })
...
test('§4.3 phrases: longest first, blanked spans, evidence verbatim', () => {
  assert.deepEqual(statusFromPhrases('SAMPLE All schools in the Central region are Closed for the Day'),
    { status: 'closed', status_basis: 'phrase', status_evidence: 'Closed for the Day' })
  ...
  assert.deepEqual(statusFromPhrases('SAMPLE Buses cancelled and schools closed today'), { status: 'other', status_basis: 'none', status_evidence: null })
  // whole words only
...
test('§4.4 region and province phrases', () => {
  assert.deepEqual(findRegionPhrase('SAMPLE All schools in the Central region are closed'),
    { scope: 'region', scope_region: 'central', scope_evidence: 'All schools in the Central region', ambiguous: false })
  ...
  assert.deepEqual(findRegionPhrase('SAMPLE All schools in the province are closed'),
    { scope: 'province', scope_region: null, scope_evidence: 'All schools in the province', ambiguous: false })
