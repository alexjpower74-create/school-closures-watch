...
export function schoolStatus (school, index, health, now) {
  ...
  const base = {
    ...
    unmatched_in_region: school.coverage === 'nlschools' && school.region
      ? (index.unmatchedByRegion.get(school.region) ?? []).filter(fresh).length
      : 0
  }
  ...
  const unknown = (reason, extra = {}, h = null) => finish('unknown', { ...extra, reason, reason_text: reasonText(reason, { health: h, now }) })
