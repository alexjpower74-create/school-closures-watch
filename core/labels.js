...
export const PHRASES = {
  ...
  closed: ['closed all day', 'closed for the day', 'closed today', 'will be closed', 'are closed', 'is closed', 'remain closed'],
}

export const CSFP_WORDS = [
  'ferme',
  'fermee',
  'fermees',
  'fermes',
...
const REGION_TEMPLATES = [
  'all schools in the {R} region',
  'all schools in {R} region',
  'all {R} region schools',
  'all {R} schools',
  '{R} region schools',
  'schools in the {R} region',
]
...
export function findPhrases(s, entries) {
  ...
  for (const e of sorted) {
    ...
    for (let i = 0; i + n <= toks.length; i++) {
      let ok = true
      for (let k = 0; k < n; k++) {
        if (used[i + k] || toks[i + k].norm !== e.words[k]) {
          ok = false
          break
        }
      }
...
export function hasCsfpWord(text) {
  return (
    findPhrases(
      text,
      CSFP_WORDS.map((phrase) => ({ phrase, value: phrase })),
    ).length > 0
  )
}
