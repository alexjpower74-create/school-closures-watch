...
const NAMED = {
  ...
  hellip: '…', eacute: 'é', egrave: 'è', agrave: 'à', ecirc: 'ê', ccedil: 'ç', ocirc: 'ô', icirc: 'î',
  laquo: '«', raquo: '»'
}
...
export function wordTokens (s) {
  ...
  for (let i = 0; i < str.length;) {
    ...
    if (APOS.has(ch)) { if (cur) cur.end = next; i = next; continue }
    if (ch === '&') { flush(); out.push({ norm: 'and', start: i, end: next }); i = next; continue }
...
export function normName (s) {
  return wordTokens(s).map(t => t.norm).join(' ')
}
