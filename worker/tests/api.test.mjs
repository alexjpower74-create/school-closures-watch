...
test('today ingest → Glovertown closed exact, Eastside other, another NLSchools school open with as_of', async () => {
  ...
  assert.deepEqual(
    res.map((r) => [r.source_id, r.accepted, r.quotes_dropped]),
    [
      ['nlschools-status', 2, 0],
      ['nlschools-notices', 0, 0],
      ['csfp-news', 0, 0],
    ],
  )
  ...
  assert.deepEqual(
    [g.school.id, g.status, g.label, g.source_status_text, g.applies.map((a) => a.how)],
    [GLOVERTOWN, 'closed', 'Closed', 'CLOSED ALL DAY', ['exact']],
  )
...
test('SAMPLE whole-Central closure → all 77 Central NLSchools schools closed how region; no Western or CSFP school affected', async () => {
  ...
  assert.ok(
    central.every((s) => s.status === 'closed' && s.applies.length && s.applies.every((a) => a.how === 'region')),
    JSON.stringify(central.find((s) => s.status !== 'closed')),
  )
  const western = await statuses(WESTERN_IDS, { now, ...S })
  assert.ok(
    western.every((s) => s.status === 'open'),
    'a Western school was affected',
  )
  ...
  assert.deepEqual(today.region_wide.map((n) => [n.source_id, n.scope, n.scope_region]).sort(), [
    ['nlschools-notices', 'region', 'central'],
    ['nlschools-status', 'region', 'central'],
  ])
...
test('SAMPLE ambiguous → Glovertown may_apply (NOT closed)', async () => {
  ...
  assert.deepEqual(
    g.may_apply.map((m) => [m.reason, m.reason_text]),
    [['name_similar', 'The notice names a similar school: "SAMPLE Glovertown".']],
  )
  const n = (await get(`/api/notices/${g.may_apply[0].notice_id}`, S)).body
  assert.deepEqual(n.applies_to, [])
  assert.deepEqual(
    n.may_apply_to.map((m) => [m.school.id, m.reason]),
    [[GLOVERTOWN, 'name_similar']],
  )
})
...
test('stale (fixture 503) → stale true + stale_text; open schools become unknown stale; Glovertown stays closed with stale true', async () => {
  ...
  assert.deepEqual(
    s2.body.sources.map((s) => [s.source_id, s.result, s.error]),
    [
      ['nlschools-status', 'error', 'HTTP 503'],
      ['nlschools-notices', 'error', 'HTTP 503'],
      ['csfp-news', 'ok', null],
    ],
  )
  ...
  assert.deepEqual(
    [src.stale, src.stale_reason, src.last_result, src.last_error, src.last_ok_at],
    [true, 'last_attempt_failed', 'error', 'HTTP 503', T0],
  )
...
test('a second ok scan without a row → removed_at set and it appears in /api/today earlier', async () => {
  ...
  assert.deepEqual(
    western.notices[0].applies_to.map((a) => [a.school_id, a.how]),
    [[EASTSIDE, 'exact']],
  )
...
test('old list date → unknown list_date_old', async () => {
  ...
  const o = statusOf(r.body.schools, OTHER_CENTRAL)
  assert.deepEqual(
    [o.status, o.reason, o.reason_text],
    ['unknown', 'list_date_old', 'The NLSchools list is still showing Monday, September 14, 2026.'],
  )
...
test('tampered quote at ingest → dropped', async () => {
  const tamper = (p) =>
    p.source_id !== 'nlschools-status'
      ? p
      : {
          ...p,
          notices: p.notices.map((n) =>
            n.school_text === 'Glovertown Academy' ? { ...n, quote: 'School open as usual NOTE: Water Shut Off' } : n,
          ),
        }
...
test('a payload cannot bring its own matches or status: the Worker re-derives them', async () => {
  const tamper = (p) =>
    p.source_id !== 'nlschools-status'
      ...
      : {
          ...p,
          notices: p.notices.map((n) => ({ ...n, status: 'open', matches: [{ school_id: OTHER_CENTRAL, how: 'exact', reason: null }] })),
        }
...
test('401 without token (ingest, scan); 400 on a bad payload', async () => {
  ...
  assert.equal(
    (await get('/api/status', { ids: GLOVERTOWN, ...S })).body.sources.find((s) => s.id === 'nlschools-status').last_result,
    'never',
  )
})
...
test('/api/admin/scan (the scheduled() code path) against the fixture origin: polite, due-only unless forced', async () => {
  ...
  assert.deepEqual(
    s1.body.sources.map((s) => [s.source_id, s.result, s.notices_current]),
    [
      ['nlschools-status', 'ok', 2],
      ['nlschools-notices', 'ok', 0],
      ['csfp-news', 'ok', 0],
    ],
  )
  let log = await fixtureLog()
  assert.deepEqual(
    log.hits.map((h) => h.path).filter((p) => p !== '/feed/'),
    ['/schools/statusreport.jsp', '/schools/generated/schoolstatus.html', '/about/generated/newspostings_5.html'],
  )
  ...
  assert.deepEqual(
    s2.body.sources.map((s) => s.result),
  ...
  assert.deepEqual(
    s3.body.sources.map((s) => s.result),
    ['ok', 'ok', 'skipped'],
  )
...
test('CSFP SAMPLE post → École Boréale may_apply; École Sainte-Anne stays unknown', async () => {
  ...
  assert.deepEqual(
    r.body.schools.map((s) => [s.school.id, s.status]),
    [
      [BOREALE, 'may_apply'],
      [SAINTE_ANNE, 'unknown'],
    ],
...
test('/api/schools, /api/sources, /api/health, CORS preflight', async () => {
  ...
  assert.deepEqual(src.sources.map((x) => [x.id, x.kind]).slice(3), [
...
test('CORS: every GET (errors and raw copies too) carries Access-Control-Allow-Origin: *; preflight allows authorization', async () => {
  ...
  const cases = [
    ['/api/health', {}, 200],
    ['/api/schools', {}, 200],
    ['/api/status', { ids: GLOVERTOWN, ...S }, 200],
    ['/api/status', { ids: '' }, 400],
    ['/api/today', S, 200],
    ['/api/sources', {}, 200],
    [`/api/notices/${gid}`, S, 200],
    ['/api/notices/nope', {}, 404],
    [raw, {}, 200],
    ['/api/raw/nlschools-status/nope.html', {}, 404],
    ['/api/nope', {}, 404],
  ]
  ...
  const pre = await fetch(`${BASE}/api/admin/ingest`, {
    method: 'OPTIONS',
    headers: {
...
test('storm seed (seed.mjs): /api/status notices map is complete; /api/today applies_to has may_apply with reason; sources fields', async () => {
  const seed = spawnSync(
    process.execPath,
    [ROOT + 'worker/tests/seed.mjs', '--url', BASE, '--token', TOKEN, '--scenario', 'storm', '--now', T0, '--reset'],
    { encoding: 'utf8' },
  )
  ...
  const ids = [
    GLOVERTOWN,
    idOf("Bay d'Espoir Academy"),
    idOf('Bishop White School'),
    idOf("St. Mark's School"),
    OTHER_CENTRAL,
    EASTSIDE,
    BOREALE,
  ]
  ...
  for (const src of r.sources)
    for (const k of ['id', 'name', 'kind', 'human_url', 'stale', 'stale_text']) assert.ok(k in src, `sources[].${k}`)
  ...
  assert.deepEqual(
    amb.applies_to.map((a) => [a.school_id, a.how, a.reason]),
    [[GLOVERTOWN, 'may_apply', 'name_similar']],
  )
  const exact = central.notices.find((n) => n.school_text === 'Bishop White School')
  assert.deepEqual(
    exact.applies_to.map((a) => [a.how, a.reason]),
    [['exact', null]],
  )
...
test('cron: scheduled() runs a scan (wrangler --test-scheduled)', async () => {
  ...
  let src
  for (let i = 0; i < 40; i++) {
    // scheduled() finishes in the background (≥ 2.2 s of polite gaps)
